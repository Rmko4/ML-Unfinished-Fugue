...
INCREASE_DMINOR_PROB_START_MEASURE = 10 #raise the probability for D_minor notes in the beginning of a measure
DECRESE__ACROSS_BAR_PROB = 0.3 #Decrease the liklyhood for notes to start before and end after a bar
D_MINOR_LEN_ADAPT = 5 #Increases liklyhood for long notes if choosen note is in Dminor. Should be above 1
...
ADAPT_PITCH_B_B = 6
ADAPT_PITCH_B = 1
ADAPT_PITCH_NOTES = [ADAPT_PITCH_A, ADAPT_PITCH_B_B, ADAPT_PITCH_B, ADAPT_PITCH_C, ADAPT_PITCH_D_B, ADAPT_PITCH_D, ADAPT_PITCH_E_B, ADAPT_PITCH_E, ADAPT_PITCH_F, ADAPT_PITCH_G_B, ADAPT_PITCH_G, ADAPT_PITCH_A_B]
...
ADAPT_LENGTH_B_B = 3
ADAPT_LENGTH_B = 1
ADAPT_LENGTH_NOTES = [ADAPT_LENGTH_A, ADAPT_LENGTH_B_B, ADAPT_LENGTH_B, ADAPT_LENGTH_C, ADAPT_LENGTH_D_B, ADAPT_LENGTH_D, ADAPT_LENGTH_E_B, ADAPT_LENGTH_E, ADAPT_LENGTH_F, ADAPT_LENGTH_G_B, ADAPT_LENGTH_G, ADAPT_LENGTH_A_B,]
...
def is_diminor(note):
    d_minor = [0,1,3,5,7,8,10] #assumes first probability corresponses to A_3 (220	55 Hz)
    for x in d_minor:
        if note % 12 ==x:
            return True
    return False
...
def update_based_on_difference_to_other_voices(prob_notes, other_voice):
    for i in range(len(prob_notes)):
        difference = abs(i - (other_voice -37)) #-37 because assumption of starting A_3 220 Hz
        if difference < len(PROB_VOICE_DIFFERENCES):
            prob_notes[i] *= 5*PROB_VOICE_DIFFERENCES[difference]**2 +1 #TODO Adapt!
    return prob_notes
